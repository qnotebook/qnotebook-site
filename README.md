...
Static site for qnotebook.org, built with [Zola](https://www.getzola.org).
...
The custom domain (`qnotebook.org`) is set in the repository's **Settings -> Pages**.
DNS: point the apex `qnotebook.org` at GitHub Pages (A/AAAA records) and set
`www.qnotebook.org` as a CNAME to `qnotebook.github.io`. Enable *Enforce HTTPS* once the
